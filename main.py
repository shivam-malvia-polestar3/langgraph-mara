...
# SESSIONS: Dict[str, List[AnyMessage]] = {}   # ordered history of messages
...
# 
# # Per-tool system prompts for summarization (HOURS for gaps to avoid unit slips)
...
# 
# def _seed_defaults_from_query(session_id: str, query: str):
#   """Greedy parse of IMO/MMSI and Month-Year ranges from the query into session defaults."""
#   ctx = SESSION_CTX.setdefault(session_id, {})
# 
#   # IMO (7 digits)
#   m_imo = re.search(r'\bIMO\s*([0-9]{7})\b', query, re.IGNORECASE)
#   if m_imo:
#     ctx["imo"] = m_imo.group(1)
# 
#   # MMSI (9 digits)
#   m_mmsi = re.search(r'\bMMSI\s*([0-9]{9})\b', query, re.IGNORECASE)
#   if m_mmsi:
#     ctx["mmsi"] = m_mmsi.group(1)
# 
#   # Explicit "with imo 9169055" without "IMO"
#   m_imo2 = re.search(r'\bwith\s+imo\s+([0-9]{7})\b', query, re.IGNORECASE)
#   if m_imo2:
#     ctx["imo"] = m_imo2.group(1)
# 
#   # Date range like "January 2025 to March 2025" or "Jan 2025 - Mar 2025"
#   m_range = re.search(
...
#     start_raw = _try_make_date(s_mon, s_year, "start")
#     end_raw   = _try_make_date(e_mon, e_year, "end")
#     if start_raw:
#       ctx["timestamp_start"] = _normalize_ts(start_raw, "start")
#     if end_raw:
#       ctx["timestamp_end"] = _normalize_ts(end_raw, "end")
#     return
# 
...
#   SESSIONS_META.setdefault(sid, {"created_at": _utcnow_iso(), "last_activity_at": _utcnow_iso(), "first_user_message": None})
#   return sid
# 
...
# 
#   # seed defaults from the natural-language query (IMO/MMSI/dates)
#   _seed_defaults_from_query(session_id, body.query)
# 
#   # build message list: persistent history + stricter system instructions
#   history = SESSIONS[session_id][:]
#   base_system = SystemMessage(content=(
#     "You are a maritime analytics assistant. "
#     "Use the session defaults (IMO/MMSI/time window) when the user omits them. "
#     "Only call the tools that are explicitly requested or are obviously required to answer the question. "
#     "Examples: if user asks only for 'gaps', do not call STS. "
#     "If a user asks by flag, call get_vessel_data(flag) to obtain IMOs, then call the requested per-IMO tools. "
#     "If an MMSI is available, include it in tool args to populate pos_url. "
#     "Prefer to issue all necessary tool calls before replying."
#   ))
...
# 
#   # Record the boundary to extract only *new* messages from this turn
#   before_len = len(msgs)
# 
#   # run graph
#   final = graph.invoke({"messages": msgs})
# 
#   # ===== extract ONLY the new messages produced by this call
#   new_msgs = final["messages"][before_len:] if len(final["messages"]) > before_len else final["messages"]
# 
#   # collect tool outputs from *this turn only*
...
# 
#   # last AI text from this turn (scan new_msgs backwards)
...
#   # Append *only* new AI/tool messages from this turn
#   for m in new_msgs:
#     if isinstance(m, (AIMessage, ToolMessage)):
#       SESSIONS[session_id].append(m)
# 
#   # Session metadata (first user message & activity)
#   meta = SESSIONS_META.setdefault(session_id, {"created_at": _utcnow_iso(), "last_activity_at": _utcnow_iso(), "first_user_message": None})
#   if meta.get("first_user_message") is None:
#     meta["first_user_message"] = body.query
#   meta["last_activity_at"] = _utcnow_iso()
# 
#   # update session defaults from tool call args
#   _update_ctx_from_tool_calls(session_id, new_msgs)
# 
#   return {
